...
Một ứng dụng phát triển ontology sẽ không còn ý nghĩa nếu không có tính năng reasoning,
Reasoner sử dụng trong UIT-OWL Editor là Pellet Reasoner \cite{pellet}. Reasoner này cũng đã được tạo sẵn trong \verb|OWLEditorKitImpl|, sử dụng thông qua getter \verb|getReasoner()|. Ví dụ:
\begin{verbatim}
Set<OWLCLass> inferredClasses = eKit
               .getReasoner().getSubClass(
\end{verbatim}
