...
\begin{figure}[h!]
	\centering
	\frame{\includegraphics[width=140mm]{Figures/owleditor_converter.png}}
...
Bên cạnh việc sử dụng đặc tính suy luận của Ontology để thực hiện phân loại, chúng em tự thiết kế một tính năng hỗ trợ phân loại dựa theo những thông tin khai báo trong SWRLRule, và dùng những ràng buộc về lĩnh vực (domain) và miền dữ liệu giới hạn (range) để hạn chế dữ liệu mà người dùng có thể lựa chọn qua đó làm giảm thời gian điền vào các mảnh thông tin còn thiếu. 
\\
Để phác họa cho ý tưởng này em xin trích một đoạn chứa các SWRLRule trong ontology \textit{Transport.owl} \cite{owleditorSrc} (được chúng em xây dựng để làm demo cho tính năng phân loại):
\begin{verbatim}
# 
OnRoadAndOffRoadVehicle(?x) ^ canCarry(?x, ?y) ^ Cargo(?y)  -> Truck(?x)
#
OnRoadAndOffRoadVehicle(?x) ^ canCarryCargo(?x, true) -> Truck(?x)
# Phương tiện đường bộ có số 4 <= chỗ ngồi <= 7 là xe hơi
