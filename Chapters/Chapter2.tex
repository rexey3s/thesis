...
\section{Ứng dụng HST để xác định tất cả kiểm chứng cho kết quả suy luận}
...
\begin{enumerate}
\item Chọn một phát biểu $\alpha$ nẳm trong nhãn của $v$ nhưng không dán nhãn một cạnh nối $v$ tới một node kế cận.
\item Gọi $S$ là hội của (union of) {$\alpha$} và tập những phát biểu dán nhãn cho những cạnh làm thành đường đi từ $v$ tới gốc cây. Loại bỏ $S$ khỏi $\beta$ ta được $\beta^{'}$.
\item Nếu $\eta$ thỏa $\beta^{'}$ $\models$ $\eta$ thì tìm một kiểm chứng (justification) $J$ cho $\eta$ trong $\beta^{'}$. Nếu $\beta^{'}$ $\not\models$ $\eta$ thì set $J$ = $\emptyset$.
\item Tạo một node mới $v^{'}$ và set nhãn của $v^{'}$ là tập $J$ ở bước trên.
\item Thêm cạnh $e$ = ($v$, $v^{'}$) vào cây và dán nhãn cho $e$ với $J$ như bước 1.
...
\end{enumerate} 
Trong đó $\eta$ $=$ $A$ $subseteq$ $D$. HST cho $\beta$ $\models$ $A$ $subseteq$ $D$ được thể hiện ở hình 2.2. Bắt đầu tại gốc cây, node được dán nhãn $J_{1}$ HST mở rộng về phía bên trái bằng cách loại bỏ $A$ $subseteq$ $B$ khỏi $\beta$ và tính toán lại kiếm chứng thỏa $\beta$ \backslash {$A$ $subseteq$ $B$} $\models$ $A$ $subseteq$ $D$. Trong trường hợp này, kiểm chứng $J_{2}$ được tìm ra. Hướng bên trái của node kế cận node gốc (root node) sau đó được dán nhãn $J_{2}$ và cạnh kết nối của nó là $A$ $subseteq$ $B$. Tương tự, node nằm ở đáy bên phía tay phải liền kề với node được dán nhãn bằng $J^{'}_{2}$ và cạnh kết nối có nhãn là $exists$ $R.T$ $subseteq$ $D$. Node này được tạo ra bằng việc xem xét $\beta$ \backslash $S$ với $S$ = {$B$ $subseteq$ $D$, $exists$ $R.\top$ $subseteq$ $D$} ($exists$ $R.T$ cộng với các nhãn của đường đi tới gốc) và $\beta$ \backslash $S$  không có bất kì một kiểm chứng nào để $A$ $subseteq$ $D$. Một node liền kề sau đó được tạo và đánh dấu chúng với tập rỗng. với cạnh kế cận được dán nhãn là $exists$ $R,\top$.
\\\hspace*{.05\textwidth} Khi không còn node kết cần để tạo ra một HST hoàn chỉnh.
